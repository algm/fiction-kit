...
function Rewrite-Paths {
    ...
    $Content = $Content -replace '(/?)\bmemory/', '.fiction/memory/'
    $Content = $Content -replace '(/?)\bscripts/', '.fiction/scripts/'
    $Content = $Content -replace '(/?)\btemplates/', '.fiction/templates/'
    return $Content
}
...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        $outputFile = Join-Path $OutputDir "fiction.$name.$Extension"
...
function Generate-CopilotPrompts {
    ...
    $agentFiles = Get-ChildItem -Path "$AgentsDir/fiction.*.agent.md" -File -ErrorAction SilentlyContinue
...
function Build-Variant {
    ...
    $specDir = Join-Path $baseDir ".fiction"
    New-Item -ItemType Directory -Path $specDir -Force | Out-Null
    
    # Copy memory directory
    if (Test-Path "memory") {
        Copy-Item -Path "memory" -Destination $specDir -Recurse -Force
        Write-Host "Copied memory -> .fiction"
    }
    ...
    if (Test-Path "templates") {
        ...
        Write-Host "Copied templates -> .fiction/templates"
    }
    ...
    # Create zip archive
    $zipFile = Join-Path $GenReleasesDir "fiction-kit-template-${Agent}-${Script}-${Version}.zip"
...
Get-ChildItem -Path $GenReleasesDir -Filter "fiction-kit-template-*-${Version}.zip" | ForEach-Object {
    Write-Host "  $($_.Name)"
}
